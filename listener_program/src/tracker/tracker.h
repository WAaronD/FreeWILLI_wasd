...
class Tracker
{
   ...
   private:
    ...
    auto getClusterCentroids(const std::vector<point3>& data,
                             const std::vector<size_t>& labels) -> std::vector<Eigen::Vector3f>;

    void incrementMissedCounter(const std::set<int>& associated_filters);

    void initializeFiltersForClusters(const std::vector<int>& unassigned_clusters,
...
class ITracker
{
   public:
    static std::unique_ptr<Tracker> create(PipelineVariables pipelineVariables)
    {
        ...
        {
            return std::make_unique<Tracker>(0.04f, 15, 4, "", pipelineVariables.clusterFrequencyInSeconds,
