 ...
 * @param dataBytes Reference to the vector that will hold the retrieved data
 * bytes.
 */
void waitForData(SharedDataManager& sess, std::vector<std::vector<uint8_t>>& dataBytes, int numPacksToGet)
{
    while (true)
    {
        ...
        bool gotData = sess.popDataFromBuffer(dataBytes,
                                              numPacksToGet);  // Retrieves data or sleeps until available
        if (gotData)
        {
            return;
        }
...
/**
 * @brief Writes error-causing timestamps and associated data bytes to the
 * standard error stream.
 *
 * This function logs the timestamps and corresponding byte data that caused an
 * error to `std::cerr`. Timestamps are converted to microseconds since the
 * epoch, and byte data is formatted in hexadecimal with zero padding for
 * readability.
 *
 * @param errorTimestamps A span of TimePoint objects representing the
 * timestamps of the errored data.
 * @param erroredDataBytes A vector of vectors containing the byte data
 * associated with the errored timestamps.
 */
