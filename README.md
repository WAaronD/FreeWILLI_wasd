...
Directories
- benchmark
Contains source code for autmated benchmark tests.

- bin
Stores compiled executables of the program.

- config_files
JSON files for specifiying runtime configuration 

- debug
Contains debug builds of the program and tests.

- deployment_files
Stores data and files produced by the program at runtime

- filters
Contains filter coefficients used for signal processing, which are loaded into the program during runtime.
...
- out
Stores build artifacts generated by the build system (e.g., CMake).

- receiver_pos
Stores receiver/hydrophone position data used for TDOA and DOA calculations.

- src
Main source code of the program.

- tests
Contains unit test source code.
