...
- ```.clang-format```: Configuration file specifying coding style rules for automatic formatting using clang-format.
- ```.clang-tidy```: Configuration file for clang-tidy, a static analysis tool for detecting potential issues in C++ code.
- ```Dockerfile```: Defines a containerized environment for building and running the project using Docker.
- ```integration_testing/```: Contains integration tests to validate interactions between different components of the project.
- ```LICENSE```: The license file specifying the terms of use, distribution, and modification of the project.
...
The following instructions apply specifically to **Linux x86_64** systems. If you are using an ARM-based Linux system (e.g., Raspberry Pi, Jetson Nano, etc.), you will need to modify the instructions.

1. Installing CMake 3.29.7 on Linux x86_64
...
```
- **<config.json>**
Path to a JSON configuration file (e.g. ```config_files/volumetric.json```) that specifies signal processing pipeline construction.  The configuration uses a hierarchical structure with:
- **network section**: Network connection parameters (IP address, port) of input data stream
- **global section**: Shared parameters accessible throughout the configuration via variable references
- **pipeline.steps** array: Ordered sequence of processing stages to execute

Each ```step``` specifies its ```type``` (corresponding to ```PipelineBuilder``` methods) and ```params``` (stage-specific parameters). The ```type``` referes to a specific type of signal processing stage and the ```params``` are used to instatiate that stage. See ```src/core/processing_stages.h``` for the list of all available signal processing stages. In ```src/core/config_parser.cpp```, the function ```FlexibleConfigParser::parse()``` parses the JSON into a ```FlexibleConfig``` structure, and ```FlexiblePipelineFactory::createPipeline()``` dynamically constructs the pipeline in a sequence of stages.
...
**Order of operations:**
1. **Parse Configuration and Resolve Variables**

The ```FlexibleConfigParser``` reads the JSON configuration, resolves all variable references (```${section.field}```), and creates a ```FlexibleConfig``` structure containing the network settings, global parameters, and ordered pipeline steps.

2. **Dynamic Pipeline Construction**
```FlexiblePipelineFactory::executeStep()``` iterates through each configured step and dynamically calls the corresponding ```PipelineBuilder``` method. The factory uses string-to-method mapping to execute:

- Processing stages (```addDataAcquisition```, ```addTimeDomainDetection```, etc.)
- Output configuration (```setFileOutput```, ```setConsoleOutput```, ```setNetworkOutput```)
...
5. **Pipeline = Infrastructure + Orchestration**  
...
- **Track Initialization & Termination**: New tracks are created when a cluster of detections persists over multiple frames, and tracks are removed if they become inactive.
...
- ```CrossCompileSettingsDynamic.cmake```: CMake configuration for cross-compiling (e.g., for ARM-based devices like Raspberry Pi) using a dynamic build.
- ```CrossCompileSettingsStatic.cmake```: CMake configuration for cross-compiling (e.g., for ARM-based devices like Raspberry Pi) using a static build.
